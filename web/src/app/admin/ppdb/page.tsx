...
import { Loader2, CheckCircle, XCircle, Clock, Eye, Trash2, FileText, Download, Lock, Unlock } from "lucide-react";
...
export default function AdminPPDBPage() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (!error && data && typeof data.is_ppdb_open === "boolean") {
        setIsPpdbOpen(data.is_ppdb_open);
      }
    } catch (e) {
      console.error("Error fetching PPDB status:", e);
    }
  };
  ...
  const handleTogglePPDB = async () => {
    const nextState = !isPpdbOpen;
    const actionName = nextState ? "MEMBUKA" : "MENUTUP";
    if (!confirm(`Yakin ingin ${actionName} pendaftaran PPDB untuk calon santri di website?`)) return;

    setToggling(true);
    try {
      const { data, error } = await supabase
        .from("settings")
        .update({ is_ppdb_open: nextState })
        .eq("id", 1)
        .select();
      if (error) {
        alert("Gagal memperbarui status PPDB: " + error.message + "\nPastikan migrasi SQL penambahan kolom is_ppdb_open sudah dijalankan.");
      } else if (!data || data.length === 0) {
        alert("Gagal menyimpan ke database Supabase!\nPerubahan diblokir oleh sistem keamanan RLS atau tabel pengaturan belum diinisialisasi.\n\nSolusi: Silakan jalankan ulang perintah SQL pada file 'supabase_ppdb_toggle.sql' di SQL Editor Supabase Studio Anda.");
      } else {
        setIsPpdbOpen(nextState);
      }
    } catch (e: any) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          disabled={toggling}
          className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-sm shadow-sm transition-all cursor-pointer ${
            isPpdbOpen 
        ...
        >
          ...
          <span>{isPpdbOpen ? "PPDB Sedang Dibuka (Tutup?)" : "PPDB Ditutup (Buka Sekarang?)"}</span>
        </button>
      </div>
