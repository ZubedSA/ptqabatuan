    ...
    const fetchData = async () => {
      ...
      if (newsItem) {
        setFormData({
          ...
          category: newsItem.category || "Pengumuman",
          is_published: newsItem.is_published,
        });
        setOriginalThumbnail(newsItem.thumbnail || "");
      }

      // Fetch Categories
      const { data: cats } = await supabase.from("news").select("category");
      if (cats) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedFile) {
      ...
      try {
        ...
        // Hapus file lama jika merupakan URL Google Drive
        if (originalThumbnail && originalThumbnail.includes("drive.google.com")) {
          await deleteFromGDrive(originalThumbnail);
        }
      } catch (error: any) {
    ...
    } else if (formData.thumbnail === "" && originalThumbnail) {
      // Jika user menghapus thumbnail yang sebelumnya ada
      if (originalThumbnail.includes("drive.google.com")) {
        await deleteFromGDrive(originalThumbnail);
      }
    }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Ubah informasi berita pesantren.</p>
        </div>
      </div>
