...
import { ArrowLeft, CheckCircle, Loader2, UploadCloud, Lock, AlertCircle } from "lucide-react";
...
export default function PPDBRegistrationPage() {
  ...
  if (checkingStatus) {
    return (
      <div className="min-h-screen pt-32 pb-12 bg-gray-50 flex items-center justify-center p-4">
        <div className="flex flex-col items-center gap-3 text-gray-500">
          <Loader2 className="w-8 h-8 animate-spin text-[#0B3B24]" />
          <p className="text-sm font-medium">Memeriksa status gerbang PPDB...</p>
        </div>
      </div>
    );
  }
  ...
  if (!isOpen) {
    return (
      <div className="min-h-screen pt-32 pb-12 bg-gray-50 flex items-center justify-center p-4 md:p-6">
        <motion.div 
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className="bg-white p-8 md:p-12 rounded-3xl shadow-xl max-w-lg w-full text-center border border-red-100"
        >
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-3">Pendaftaran PPDB Ditutup</h2>
          <p className="text-sm md:text-base text-gray-600 mb-8 leading-relaxed">
            Mohon maaf, pengisian formulir pendaftaran santri baru PTQA Batuan saat ini sedang ditutup atau belum dibuka. Silakan kembali lagi di lain waktu.
          </p>
