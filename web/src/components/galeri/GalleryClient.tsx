...
import { Loader2, X, Image as ImageIcon, ZoomIn, Share2 } from "lucide-react";

export default function GalleryClient({ initialId }: { initialId?: string }) {
  ...
  const handleShareItem = (item: any, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    const shareUrl = `${window.location.origin}/galeri?id=${item.id}`;
    const shareText = `${item.title}\n\nDokumentasi Galeri PTQA Batuan:\n`;
                ...
                >
                  ...
                  <div className="p-5 md:p-6 flex flex-col flex-1">
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <span className="inline-block px-2.5 py-1 bg-[#D4AF37]/10 text-[#D4AF37] text-[10px] font-bold uppercase tracking-wider rounded-md">
                        {item.category}
                      </span>
                      <button
                        onClick={(e) => handleShareItem(item, e)}
                        className="inline-flex items-center gap-1 px-2.5 py-1 bg-[#25D366]/10 hover:bg-[#25D366] text-[#25D366] hover:text-white text-[11px] font-bold rounded-lg transition-all"
          ...
          >
            ...
            >
              ...
              <div className="w-full mt-4 text-center">
                ...
                <button
                  onClick={() => handleShareItem(selectedImage)}
                  className="inline-flex items-center gap-2 px-6 py-2.5 bg-[#25D366] hover:bg-[#20ba5a] text-white font-bold text-sm rounded-full transition-all shadow-lg cursor-pointer"
                >
                  <Share2 className="w-4 h-4" />
                  <span>Share Gambar Ini ke WhatsApp</span>
                </button>
              </div>
            </motion.div>
          </motion.div>
