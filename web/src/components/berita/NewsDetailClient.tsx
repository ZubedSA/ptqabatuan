...
export default function NewsDetailClient({ slug }: { slug: string }) {
  ...
  const handleShareWhatsApp = () => {
    const shareText = `${newsItem.title}\n\nBaca berita selengkapnya di PTQA Batuan:\n`;
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-background pt-20">
        ...
        <p className="text-gray-500 font-medium animate-pulse">Memuat berita...</p>
      </div>
    );
  }
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-4 text-white/80 text-sm font-medium">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {Math.max(1, Math.ceil((newsItem.content || "").split(" ").length / 200))} Menit Membaca
                </div>
              </div>
            </motion.div>
          </div>
        ...
        <section className="py-16 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white font-serif mb-10 text-center">
              Berita Terkait di <span className="text-primary">{newsItem.category}</span>
            </h3>
            
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {relatedNews.map((related) => (
                <Link key={related.id} href={`/berita/${related.slug}`} className="group flex flex-col bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl border border-gray-100 dark:border-gray-700 transition-all">
